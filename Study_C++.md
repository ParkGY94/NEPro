...
## 8. 옵저버 패턴
### 옵저버 패턴은 옵저버들이 관찰할 수 있는 대상자의 상태가 변화가 있을 때마다 대상자는 직접 목록의 각 관찰자들에게 통지하고, 관찰자들은 알림을 받아 조치를 취하는 행동 패턴이다.
### 옵저버 패턴은 다른 디자인 패턴들과는 다르게 일대다 의존성을 가지는데, 주로 분산 이벤트 핸들링 시스템을 구현하는데 사용된다. 발행/구독 모델로도 알려져 있다.
